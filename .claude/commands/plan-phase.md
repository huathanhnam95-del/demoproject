...
@C:/Users/Admin/.codex/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
PHASE_INFO=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
**If found:**
1. Read template: `C:/Users/Admin/.codex/get-shit-done/templates/VALIDATION.md`
