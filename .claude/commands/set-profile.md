...
```
if {{GSD_ARGS}}.profile not in ["quality", "balanced", "budget"]:
  Error: Invalid profile "{{GSD_ARGS}}.profile"
...
```bash
node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config-ensure-section
...
```json
{
  "model_profile": "{{GSD_ARGS}}.profile"
}
```
...
```
✓ Model profile set to: {{GSD_ARGS}}.profile
