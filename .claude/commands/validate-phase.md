...
```bash
AUDITOR_MODEL=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read C:/Users/Admin/.codex/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `C:/Users/Admin/.codex/get-shit-done/templates/VALIDATION.md`
...
node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" commit-docs "docs(phase-${PHASE}): add/update validation strategy"
```
