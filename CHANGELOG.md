...
### Added & Enhanced
- **PTE Speaking & Pronunciation Scoring Architecture V3 (BEL P0–P8)**:
  - **P0 Baseline & Attempt Constraints (`functions/src/practice-attempts/attempt-constraints.js`)**: Configured Cloud Tasks infrastructure and bounded practice attempt durations and rate ceilings across speaking modes.
  - **P1 Audio Timing Contracts & Azure Normalizer (`backend/local_server/segment_contract.py`, `functions/src/services/azure-speech/normalize.js`, `functions/src/services/pronunciation-assessment-service.js`, `public/pronunciation-analyzer/praat-api.js`)**: Implemented monotonic sample-accurate boundary contract (`SampleSpan`, `BoundaryRange`, `validate_timing_envelope`). Hardened null-safe scoring accessors in Azure response normalization and preserved candidate phoneme distributions. Deprecated destructive `calibrateBoundaries` endpoint shaving, making un-trimmed raw boundaries primary while isolating legacy playback in `legacy_trimmed_playback`. Added honest `'unrateable'` status with reason `'STRESS_VERIFICATION_MISSING'`.
  - **P2 AI Credits Architecture, Lifetime Wallet & Durable Scoring Jobs (`functions/src/ai-credits/*`, `functions/src/ai-scoring/*`, `public/js/ai-credit-confirmation.js`)**: Zero-AI quote math calculating credits by duration (15 credits/min for RA/RS, 25 credits/min for RL/SGD/RTS; fixed packages for writing). Lifetime per-account balance model with 5,000 default credit grant for all new accounts, top-up purchase capability, and atomic two-phase reservation/capture/release settlement. Durable scoring job state machine with optimistic lease fencing (`leaseVersion`), heartbeat renewal, and unmetered fallback. Accessible WCAG 2.1 AA credit confirmation modal with focus trapping and transparent balance itemization.
  - **P3 Ending-Safe Playback & Audio LRU Cache (`functions/src/services/azure-speech/word-clip-policy.js`, `public/pronunciation-analyzer/segment-audio-player.js`, `public/js/audio/segment-playback-coordinator.js`, `public/crm-entrance-test-result.js`)**: Eliminated heuristic word boundary shaving, flagging abutting words as uncertain with broader context spans. Exact sample Web Audio PCM slicing on `AudioBuffer`. Monotonic `LruAudioBufferCache` capped at 32 MiB and 3 recordings with active playback pin protection. Replaced stacked -25ms/-35ms trims in CRM entrance test results.
  - **P4 Continuous Assessment Pipeline for RA & RS (`functions/src/services/azure-speech/continuous-assessment.js`)**: Multi-segment reference alignment, ordered chunk comparison, and non-destructive `clipTiming` preservation across speech segments.
  - **P5 Two-Pass ASR Engine & UI Transcript Disclosure for Spoken Responses (`functions/src/services/azure-speech/two-pass-asr.js`, `public/js/audio/transcript-disclosure.js`)**: Decoupled unconditioned Pass 1 speech-to-text from Pass 2 forced alignment for open-ended modes (RL, SGD, RTS). Reference freezing to eliminate hallucination drift and null completeness scoring for unscripted speech. UMD transcript disclosure component with collapsible transcript and color-coded acoustic tokens (`token-pass`, `token-amber`, `token-red`, `token-uncertain`).
  - **P6 V4.2 Acoustic Candidate & Boundary Refinement (`backend/phoneme_service/boundary_refinement.py`, `backend/local_server/pronounce_v42.py`, `backend/local_server/server.py`)**: Deterministic STFT spectral flux and RMS energy contouring with dynamic programming Viterbi-style monotonic path selection for phonetic boundary calibration. Sibling `timingVariants.v42` envelope integration.
  - **P7 Word-Stress Coach (`backend/local_server/stress_features_v2.py`, `backend/local_server/stress_evaluator_v2.py`)**: Magnitude-preserving contrast calculation across duration (log ratio), intensity (dB delta), and F0 pitch (semitone distance) with unvoiced frame masking. Calibrated 5-class stress evaluator (`correct_primary`, `missing_primary`, `misplaced_primary`, `excess_secondary`, `unrateable`), excluding monosyllables (`status: 'not_applicable'`).
  - **P8 Staged Release, Rollout Flags & Rollback Controls (`functions/src/config/rollout-flags.js`, `src/server/rollout-flags.js`, `functions/src/public-feature-config.js`, `src/server/public-feature-config.js`)**: Centralized rollout configuration exposing 10 feature flags via `/api/config` for real-time frontend gating and rollback safety.
- **Frontend Practice Mode AI Credit Gating & Audio Pipeline Integration (`public/read-aloud-mode.js`, `public/sgd-mode.js`, `public/rts-mode.js`, `public/rl-spoken-response.js`, `public/take-notes-mode.js`, `public/index.html`)**:
  - Preflight quote confirmation before starting expensive assessment pipelines across Read Aloud, SGD, and RTS.
  - Dual audio path separation in SGD: raw original recording (`originalRecordingBlob`) preserved for backend scoring while DSP-enhanced audio (`enhancedPlaybackBlob` with 80Hz rumble filter, 16kHz resampling, and -3dBFS normalization) is used for learner playback.
  - Integrated `TranscriptDisclosure` rendering in SGD and RTS feedback views.
  - Decoupled `RLSpokenResponseController` for Retell Lecture: full backwards compatibility for notes-only submissions (`responseKind: 'notes_only'`) without requiring microphone permissions or Azure credits, with opt-in spoken retelling (`responseKind: 'spoken_retelling'`).
