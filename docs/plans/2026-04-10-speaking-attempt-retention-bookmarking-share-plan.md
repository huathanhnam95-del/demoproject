...
- Current CRM does not support student-access expiry today. Extend it before implementation.
- Add a practice-access window to enrollment records:
  - `practiceAccessStartAt`
  - `practiceAccessEndAt`
- Add a student-level override:
  - `practiceAccessOverride.mode: inherit | force_active | force_inactive`
  - `practiceAccessOverride.startAt`
  - `practiceAccessOverride.endAt`
  - `practiceAccessOverride.note`
...
- Bookmark rules:
  ...
  - If a bookmark is removed and the 10-day window has already passed, give a short grace period before purge.
...
- Add a nightly reconciliation job to re-evaluate entitlement changes and catch missed promotions or stale claim state.
