...
## Continued root functional verification, 9 September 2026

The continuation ran against clean `2784860006a3540f41f63ed4291c642a5a551092`, whose production bytes are identical to `0b029e67f5210105aaaad28226bad3e49565b19f`. This checkpoint changes documentation only. Earlier failures and the preceding partial-cohort account above remain historical evidence.

Fresh Projects lifecycle verification passed 7/7 through actual emulator Auth/Firestore, current HTTP routes and two Chrome accounts. It includes committed-draft/task reload before authenticated grouped Undo and a second reload. Its speech evidence is engineering/synthetic. Fresh Data Input canonical lifecycle verification passed 8/8, including atomic confirmation, rollback/retry, revoked/stale proof rejection and two actual client-code resume instances returning the same saved receipt without another save. That suite uses synthetic engineering speech and a canonical service bridge, not native providers or browser HTTP. Data Input has no post-save Undo endpoint.

The exact native rename smoke `paid-278486-semantic-01` passed through three actual provider stages. Together with the retained first G04 trial, there are two independent exact native rename observations. G04 trial 2 and H diagnostic trial 1 remain failed; the latter produced a nonmatching action. No blind 20-trial restart or pooled p95 was performed. The semantic smoke's qualified first-audio observation was 2170.729 ms, above the 2000 ms target; native performance acceptance remains unproven.

Native spoken confirmation `paid-confirm-278486-02` passed with exactly one real Live request and one real ASR request, no proposal request. A setup-only deterministic draft was restored and visibly previewed; actual native server proof bound to that preview authorized one normal reference-only HTTP apply. The saved task and committed draft survived reload. Authenticated endpoint Undo restored the complete original task; the second reload and unchanged project, sections, other tasks and prior operations were verified. Both quota stages finalized, and Undo changed neither used/reserved credits nor settled/pending provider money. This verifies controlled synthetic English PCM through real providers, not a physical microphone, Vietnamese recognition, history-button Undo or a complete latency cohort.

The external confirmation harness retained two failed nonpaid fake-file capture runs, corrected fixture start timing, and passed the delayed-connector nonpaid lifecycle. Its first native attempt completed Live but stopped before ASR at the conservative pending-usage guard. The prospective external-only fix reserves the full already-approved Live capacity before dispatch while preserving actual wire context and actual metering; pending-usage rules are unchanged. The revised harness passed 33/33 focused offline tests and another complete nonpaid lifecycle before the successful native attempt. No production source was changed by these harness repairs.

The final conservative test obligation is 1,519,753,750 nanoUSD (USD 1.51975375), including every prior charge/hold and an explicitly separate 7,125,750 nanoUSD contingency for the failed confirmation attempt. The successful attempt added 165,000 settled nanoUSD and retained its full 30,848,250 nanoUSD Live hold. Unknown billing remains unknown; the contingency is not an invoice charge or a ledger mutation. The independent USD 5 total authorization was respected. No further provider request was made for billing reconciliation.

Evidence is retained under `C:/Users/Admin/Documents/Codex/2026-09-08/crm-projects-recovery/work/final-acceptance-package`: `usage-credits/lifecycle-verification.json`, `usage-credits/data-input-lifecycle-verification.json`, `usage-credits/native-semantic-verification.json`, `usage-credits/native-observations-current.json`, and `latency/native-confirmation/paid-confirm-278486-02-root-review.json`. The executable root confirmation audit independently checked PCM digests, source hashes, native proof consumption, exact receipt/domain changes, reload results and accounting invariance. The continuation delivery index supersedes the earlier scoped index without deleting it. Original candidate `813015aea8826e15f0548a3102938694e4002b08` remains untouched. All owned runtimes were cleaned; no push, deployment or production migration occurred.
