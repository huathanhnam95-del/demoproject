...
4. For the live browser gate, use `tests/browser/bel-demo-online/production_rehearsal.py --channel chrome --four-player` with an external production-approved fixture, per-scenario approval/lease files, a fresh deployed-identities readback, and a final candidate manifest binding the full 40-character candidate SHA to raw local candidate bytes. The CRM step must click More, wait for `#crm-presentation-demo-workspace`, and open the popup only through `#crm-presentation-demo-open`. The scenario must observe four-player authored progression, reconnect with the same room/scene/slot state, three persisted Vietnamese participant notes after reload, room end, and both presenter-participant and participant-own PDFs with extracted text and privacy assertions. Diagnostics are retained separately for each CRM, lobby, and game page. Physical four-computer confirmation remains required. Before every approved live scenario, reread the approval, lease and deployed-identities file; reject revoked/expired/non-finite approvals, ownership/resource changes, stale or mismatched deployed state hashes, and candidate manifest SHA/byte mismatches. The exact live entrance annotation stylesheet is preserved as a 2,951-byte baseline before the candidate's additive rule; the Chrome gate verifies a 200 `text/css` response and the loaded `.et-annotation-overlay` and `.et-annotation-capture-layer` rules without suppressing MIME failures.
