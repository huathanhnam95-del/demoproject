...
## Approved monthly usage-credit successor — September 9, 2026

Covered native CRM AI services share one UID/Vietnam-month usage-credit pool, default 5,000 credits calibrated to USD 5. Server activity/text/processing metering controls quota independently of provider invoice precision. Existing money records, pending holds, eligibility, confirmation and dispatch idempotency remain intact; current-month historical consumption is imported without resetting or double counting. No unused credits roll over. Approximate voice minutes are server-calculated estimates. This product quota does not replace the separate USD 5 total paid-test guard and does not guarantee an invoice cap. The [approved implementation and migration plan](../plans/2026-09-09-shared-usage-credits.md) defines the successor contract. Implementation is underway; integrated acceptance and deployment are pending.
