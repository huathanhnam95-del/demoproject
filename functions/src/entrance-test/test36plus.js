...
function normalizeAsrContentType(contentType) {
    const raw = String(contentType || '').trim();
    const ct = raw.toLowerCase().split(';')[0].trim();

    // Hugging Face inference router rejects `audio/mp4` but accepts `audio/m4a`.
    if (ct === 'audio/mp4' || ct === 'video/mp4') return 'audio/m4a';

    // Normalize m4a aliases/parameters to the canonical type.
    if (ct === 'audio/x-m4a') return 'audio/m4a';

    // Normalize mp3 alias to the canonical type.
