...
function classifyEvent(event, referenceWords, azureWords, referenceText, audioQuality, alignment) {
  ...
  if (event.family === 'weak_form_reduction') {
    ...
    const nextIdx = alignment
      ? (alignment.get(Number(event.contextWordIndex ?? (Number(event.startWordIndex) + 1))) ?? Number(event.contextWordIndex ?? (Number(event.startWordIndex) + 1)))
      : Number(event.contextWordIndex ?? (Number(event.startWordIndex) + 1));
