...
function createPresentationDemoHandlers({ roomService, connections, notes, archives, pdf, resolveIdentity = req => req.user } = {}) {
    ...
    function failResponse(res, error) {
        ...
        const status = ['UNAUTHORIZED', 'INVALID_IDENTITY', 'ACCOUNT_DISABLED', 'ACCOUNT_INACTIVE'].includes(code) ? 401 : ['PRESENTER_ONLY', 'ACTOR_MISMATCH', 'NOTE_FORBIDDEN', 'EXPORT_FORBIDDEN'].includes(code) ? 403 : ['ROOM_NOT_FOUND', 'ARCHIVE_NOT_FOUND'].includes(code) ? 404 : 400;
    ...
    return {
        ...
        heartbeat: handler(async (req, res) => ok(res, await connections.heartbeatById(req.body?.connectionId))),
        input: handler(async (req, res) => ok(res, await connections.inputById(req.body?.connectionId, req.body?.command))),
        disconnect: handler(async (req, res) => ok(res, await connections.closeById(req.body?.connectionId))),
