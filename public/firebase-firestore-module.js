...
async function getUserProfile(userId) {
  ...
  try {
    ...
    if (!userDoc.exists()) {
      ...
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        log.log('User profile not in Firestore, using localStorage cache');
        return {
          success: true,
          data: JSON.parse(cached),
          fromCache: true
    ...
    try {
      ...
      log.warn('Could not cache user profile:', cacheError);
    }
    ...
    try {
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        log.debug('Firestore error, using localStorage cache for user profile');
      ...
      log.warn('Could not read cached profile:', cacheError);
    }
...
async function recordSessionStart(userId) {
  try {
    ...
    log.log('✓ Session started:', sessionRef.id);
    return sessionRef.id;
  } catch (error) {
    log.error('Error recording session start:', error);
    throw error;
  }
}
...
async function recordSessionEnd(sessionId, userId) {
  try {
    ...
    log.log('✓ Session ended:', sessionId, 'Duration:', duration, 'seconds');
    ...
    log.error('Error recording session end:', error);
    return {
      success: false,
      error: error.message
    };
...
async function recordPracticeAttempt(userId, questionId, isCorrect, mode = 'type') {
  try {
    ...
    // 2. Update summary stats in user document
    const userRef = doc(db, 'users', userId);
    const statsUpdate = {
      'practiceStats.total': increment(1)
    };
    if (isCorrect) {
      statsUpdate['practiceStats.correct'] = increment(1);
    } else {
      statsUpdate['practiceStats.incorrect'] = increment(1);
    }
    ...
    log.log('✓ Practice attempt recorded and stats updated atomically:', { userId, isCorrect, mode });
...
async function getPracticeStats(userId) {
  try {
    ...
    const userData = userDoc.data();
    const stats = userData.practiceStats || { total: 0, correct: 0, incorrect: 0 };

    const total = stats.total || 0;
    const correct = stats.correct || 0;
    const incorrect = stats.incorrect || (total - correct);
    ...
    log.error('Error getting practice stats from summary:', error);
    return {
      success: false,
      error: error.message
    };
...
async function getProgressStatus(userId, questionId, mode) {
  try {
    ...
    log.error('Error getting progress status:', error);
    return {
      success: false,
      error: error.message,
      progress: null
...
async function recordAttempt(userId, questionId, mode) {
  try {
    ...
    log.debug('✓ Attempt recorded:', { userId, questionId, mode, attemptCount: newAttemptCount });
    ...
    log.error('Error recording attempt:', error);
    return {
      success: false,
      error: error.message
    };
...
async function incrementProgress(userId, questionId, mode) {
  try {
    ...
    log.debug('✓ Progress incremented:', { userId, questionId, mode, newCount, newState });
    ...
    log.error('Error incrementing progress:', error);
    return {
      success: false,
      error: error.message
    };
...
async function resetProgress(userId, questionId, mode) {
  try {
    ...
    log.debug('✓ Progress reset:', { userId, questionId, mode });
...
async function getAllProgressForMode(userId, mode) {
  try {
    ...
    log.debug(`✓ Loaded ${Object.keys(progressMap).length} progress entries for ${mode} mode`);
    ...
    log.error('Error getting all progress for mode:', error);
    return {
      success: false,
      error: error.message,
      progressMap: {}
...
async function getRecentProgress(userId, mode, limitCount = 5) {
  try {
    ...
    log.error('Error getting recent progress:', error);
    return {
      success: false,
      error: error.message,
      recentProgress: []
...
async function addPoints(userId, points, title, description = '') {
  try {
    ...
    // ═══════════════════════════════════════
    if (!userId) {
      log.warn('⚠ Points not added: User not logged in');
    ...
    for (const key of inFlightRequests) {
      if (key.startsWith(dedupKey)) {
        log.debug('⏭ Points skipped: Request already in flight');
        return {
          success: true,
          skipped: true,
    ...
    // ═══════════════════════════════════════
    if (newTotal < 0) {
      log.debug(`⚠ Points floor applied: ${newTotal} → 0`);
      newTotal = 0;
    }
    ...
    // Note: absolute totals in history might be slightly out of sync if multiple updates happen,
    // but we avoid a read operation here.
    ...
    log.debug('✓ Points awarded (increment):', { userId, points, title });
...
async function getTotalPoints(userId) {
  try {
    ...
    log.error('Error getting total points:', error);
    return {
      success: false,
      error: error.message
    };
...
async function getPointsHistory(userId, limitCount = 10) {
  try {
    ...
    log.debug(`✓ Loaded ${history.length} points history entries for user ${userId}`);
    ...
    log.error('Error getting points history:', error);
    return {
      success: false,
      error: error.message,
      history: []
...
async function getPointsRules() {
  try {
    ...
    log.debug(`✓ Loaded ${rules.length} active points rules`);
    ...
    log.error('Error getting points rules:', error);
    return {
      success: false,
      error: error.message,
      rules: []
...
async function getPointsRuleByTitle(title) {
  try {
    ...
    log.error('Error getting points rule by title:', error);
    return {
      success: false,
      error: error.message,
      rule: null
...
async function awardPointsByRule(userId, ruleTitle, options = {}) {
  try {
    ...
    if (preventDuplicate !== 'none') {
      ...
      if (isDuplicate) {
        log.debug(`⏭ Points skipped (duplicate): "${ruleTitle}"`, preventDuplicate);
        return {
          success: true,
          skipped: true,
    ...
    if (!ruleResult.rule) {
      log.warn(`⚠ Points rule not found: "${ruleTitle}"`);
      return {
        success: false,
    ...
    log.error('Error awarding points by rule:', error);
    return {
      success: false,
      error: error.message
    };
...
async function awardPoints(ruleTitle, options = {}) {
  ...
  if (!userId) {
    log.debug('⏭ Points not awarded: User not logged in');
...
async function updateUnlockedModes(userId, modes) {
  try {
    ...
    log.error('Error updating unlocked modes:', error);
    return { success: false, error: error.message };
  }
}
...
/**
 * Call submitAttempt Cloud Function (server-authoritative scoring)
