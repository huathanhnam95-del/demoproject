...
(function () {
  ...
  const EXCEL_PATH = '/database/HCS/HCS/HCS.xlsx';
  const MANIFEST_PATH = '/database/HCS/audio/manifest.json';
  ...
  function parseMarkdownInHtml(html) {
    ...
    parsed = parsed.replace(/`(.*?)`/g, '<span class="hcs-inline-code">$1</span>');
    return parsed;
  }
  ...
  function appendScoreDescription(container, score, total) {
    ...
    descDiv.className = 'hcs-result-desc';
  ...
  function cacheElements() {
    ...
    elements.prevBtn = document.getElementById('hcs-v7-prev-btn');
    elements.nextBtn = document.getElementById('hcs-v7-next-btn');
    elements.questionPill = document.getElementById('hcs-v7-question-pill');
    elements.backdrop = document.getElementById('hcs-v7-backdrop');
    elements.sheet = document.getElementById('hcs-v7-sheet');
    elements.sheetClose = document.getElementById('hcs-v7-sheet-close');
    elements.jumpSearch = document.getElementById('hcs-v7-jump-search');
    elements.jumpList = document.getElementById('hcs-v7-jump-list');
    ...
    elements.passageText = document.getElementById('hcs-passage-text'); // audio transcript text container
    elements.questionPrompt = document.getElementById('hcs-question-prompt');
    elements.choicesContainer = document.getElementById('hcs-choices-container');
  ...
  function changeVoice() {
    ...
    const audioUrl = `/database/HCS/audio/${qId}/${voiceMeta.file}`;
    ...
    const restoreState = () => {
      ...
      elements.audioElement.playbackRate = state.currentSpeed;
      if (wasPlaying) {
        elements.audioElement.play().catch((err) => console.log('[HCSMode] Switch play failed:', err));
      }
  ...
  async function loadData() {
    ...
    try {
      ...
      if (manifestRes.ok) {
        ...
        console.warn('[HCSMode] Manifest res failed, falling back to empty manifest');
        state.audioManifest = {};
      }
    } catch (err) {
      console.error('[HCSMode] Error fetching manifest.json:', err);
      state.audioManifest = {};
    }
    ...
    state.questions = rawData.map((row) => {
      ...
      if (!isValid) {
        console.warn('[HCSMode] Skipping invalid workbook row:', question);
      }
      return isValid;
    }).sort((a, b) => a.id - b.id);
  }
  ...
  function loadQuestion(index) {
    ...
    if (window.PracticeRouter && state.currentQuestion?.id != null) {
      window.PracticeRouter.replaceRoute('hcs', state.currentQuestion.id);
    }
  }
  ...
  function renderQuestion() {
    ...
    if (elements.questionPrompt) {
      elements.questionPrompt.textContent = state.currentQuestion.question || 'Select the correct option that best relates to the recording.';
    }
    ...
    if (elements.choicesContainer) {
      ...
      state.shuffledChoices.forEach((choice, idx) => {
        ...
        card.type = 'button';
        card.className = 'hcs-choice-card';
        card.dataset.index = idx;
        card.setAttribute('aria-pressed', 'false');
        
        const radio = document.createElement('div');
        radio.className = 'hcs-choice-radio';
        card.appendChild(radio);

        const textDiv = document.createElement('div');
        textDiv.className = 'hcs-choice-text';
  ...
  function selectChoice(idx) {
    ...
    if (elements.choicesContainer) {
      const cards = elements.choicesContainer.querySelectorAll('.hcs-choice-card');
  ...
  function submitAnswers() {
    ...
    if (elements.resultBox) {
      ...
      headerDiv.className = `hcs-result-header ${titleClass}`;
  ...
  async function activate() {
    console.log('[HCSMode] activate called');
    cacheElements();
    ...
    if (state.questions.length === 0) {
      ...
      try {
        ...
        console.error('[HCSMode] Error loading excel database:', error);
    ...
    if (state.questions.length > 0) {
      ...
      if (urlRoute && urlRoute.mode === 'hcs' && urlRoute.questionId) {
