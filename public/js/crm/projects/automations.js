(function (globalScope) {
  ...
  function createController(deps = {}) {
    ...
    const simpleEligible = definition => !!(R.isBlankDefinition?.(definition) || R.isSimpleDefinition?.(definition));
    const builderEligible = definition => !!(simpleEligible(definition) || (!rule && R.isSimpleBuilderDraft?.(definition)));
    const advancedStatus = definition => `Advanced editor required: ${R.simpleDefinitionReason?.(definition) || 'this definition is not a simple one-trigger, one-action rule.'}`;
    ...
    function changed(next) { draft = clone(next); if (!builderEligible(draft.definition) && representation === 'recipe') { representation = 'blocks'; picker = ''; pickerQuery = ''; } generation++; dirty = !rule || JSON.stringify(draft.definition) !== baseDefinition || draft.actorUid !== version?.actorUid; preview = null; seq.preview++; status = builderEligible(draft.definition) ? 'Unsaved changes.' : advancedStatus(draft.definition); render(); }
    ...
    async function openRule(id, { preserveDraft = false, versionId = '' } = {}) {
      ...
      try {
        ...
        const builderEligible = simpleEligible(version.definition);
        representation = builderEligible ? 'recipe' : 'blocks'; picker = ''; pickerQuery = '';
        conflict = false; mode = 'edit'; opened = true; status = preserveDraft ? `Current revision loaded. Review your retained draft, save, then preview.${builderEligible ? '' : ` ${advancedStatus(version.definition)}`}` : diagnostics.length ? `Some references need repair. Replace unavailable selections below.${builderEligible ? '' : ` ${advancedStatus(version.definition)}`}` : builderEligible ? 'Automation loaded.' : advancedStatus(version.definition);
    ...
    function setRepresentation(value) { if (!['recipe', 'blocks'].includes(value)) return; if (value === 'recipe' && draft && !builderEligible(draft.definition)) { representation = 'blocks'; status = advancedStatus(draft.definition); render(); return; } representation = value; render(); }
