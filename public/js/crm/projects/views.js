(function (globalScope) {
    ...
    function createController(deps) {
        ...
        function renderTask() {
            ...
            const dateSummary = (task.startDate && task.dueDate) ? `${escape(task.startDate)} – ${escape(task.dueDate)}` : (task.dueDate ? `Due ${escape(task.dueDate)}` : (task.startDate ? `Start ${escape(task.startDate)}` : 'Dates — Not set'));
            const depCount = array(task.predecessorTaskIds).length;
            const depSummary = depCount ? `${depCount} linked task${depCount > 1 ? 's' : ''}` : 'None';
            const linkCount = array(links).length;
            const linkSummary = linkCount ? `${linkCount} link${linkCount > 1 ? 's' : ''}` : 'None';
            const taskIdEsc = escape(task.id);
            const taskRevEsc = escape(task.revision ?? 0);

            const prevDisclosures = target.querySelectorAll('details.crm-detail-disclosure');
            const hasPrev = prevDisclosures.length > 0;
            const openMap = new Map();
            if (hasPrev) {
                prevDisclosures.forEach((d) => {
                    const title = d.querySelector('.crm-detail-disclosure-title')?.textContent?.trim();
                    if (title) openMap.set(title, d.open);
                });
            }
            const prevAdvOpen = target.querySelector('details.crm-detail-advanced-dep')?.open;

            const schedOpen = hasPrev ? (openMap.get('Schedule & Timeline') ?? true) : true;
            const depOpen = hasPrev ? (openMap.get('Dependencies') ?? false) : false;
            const linkOpen = hasPrev ? (openMap.get('CRM Links') ?? false) : false;
            const infoOpen = hasPrev ? (openMap.get('Task info') ?? false) : false;
            const advOpen = prevAdvOpen ? ' open' : '';

            target.innerHTML = `<details class="crm-detail-disclosure"${schedOpen ? ' open' : ''}><summary><span class="crm-detail-disclosure-title">Schedule &amp; Timeline</span><span class="crm-detail-disclosure-badge">${dateSummary}</span></summary><div class="crm-detail-disclosure-content"><div id="projects-task-derived">${derived(task, true)}${warningList(task.dependencyWarnings)}${warningList(task.calendarWarnings)}</div><form id="projects-task-schedule" class="crm-detail-schedule-form"><div class="crm-detail-date-grid"><div class="crm-detail-field"><label for="projects-task-start" class="crm-detail-field-label"><span class="crm-field-icon">${PJ_ICON.calendar}</span> Proposed start</label><input id="projects-task-start" type="date" class="crm-input" value="${escape(task.startDate || '')}"${canWrite() ? '' : ' disabled'}></div><div class="crm-detail-field"><label for="projects-task-due" class="crm-detail-field-label"><span class="crm-field-icon">${PJ_ICON.flagDue}</span> Proposed due</label><input id="projects-task-due" type="date" class="crm-input" value="${escape(task.dueDate || '')}"${canWrite() ? '' : ' disabled'}></div></div><div class="crm-detail-form-actions"><button type="submit" class="crm-btn-secondary crm-btn-sm"${canWrite() ? '' : ' disabled'}><span class="crm-btn-icon">${PJ_ICON.bolt}</span> Preview dates</button></div></form><div id="projects-task-preview"></div></div></details><details class="crm-detail-disclosure"${depOpen ? ' open' : ''}><summary><span class="crm-detail-disclosure-title">Dependencies</span><span class="crm-detail-disclosure-badge">${depSummary}</span></summary><div class="crm-detail-disclosure-content"><div class="crm-detail-section-subtitle">Finish-to-start predecessors</div><form id="projects-task-dependencies" class="crm-detail-dependencies-form"><div class="crm-detail-dep-picker-row"><div class="crm-detail-dep-select-wrap"><select id="projects-task-predecessor-picker" aria-label="Choose predecessor task" class="crm-input"><option value="">Choose a task</option>${predecessorOptions().map((t) => `<option value="${escape(t.id)}">${escape(t.title || 'Untitled task')}</option>`).join('')}</select></div><button id="projects-task-predecessor-add" type="button" class="crm-btn-secondary crm-btn-sm"${canWrite() ? '' : ' disabled'}>+ Add predecessor</button></div><div class="crm-detail-chips-wrapper"><div class="crm-detail-chips-label">Active Predecessors:</div><div id="projects-task-predecessor-chips" class="crm-detail-chips-list"></div></div><div class="crm-detail-hint"><span class="crm-hint-icon">${PJ_ICON.info}</span><span>Predecessors must finish before this task starts. Saving dependencies will not shift existing dates.</span></div><details class="crm-detail-advanced-dep"${advOpen}><summary class="crm-detail-advanced-summary">Manual task ID entry</summary><div class="crm-detail-advanced-content"><label for="projects-task-predecessors" class="crm-detail-field-label">Predecessor task IDs (one per line):</label><textarea id="projects-task-predecessors" class="crm-input crm-dep-textarea" rows="2"${canWrite() ? '' : ' disabled'}>${escape(array(task.predecessorTaskIds).join('\n'))}</textarea></div></details><div class="crm-detail-form-actions"><button type="submit" class="crm-btn-primary crm-btn-sm"${canWrite() ? '' : ' disabled'}>Save dependencies</button></div></form><p id="projects-task-status" role="status" class="crm-task-status-banner"></p></div></details><details class="crm-detail-disclosure"${linkOpen ? ' open' : ''}><summary><span class="crm-detail-disclosure-title">CRM Links</span><span class="crm-detail-disclosure-badge">${linkSummary}</span></summary><div class="crm-detail-disclosure-content"><div id="projects-task-links"></div></div></details><details class="crm-detail-disclosure"${infoOpen ? ' open' : ''}><summary><span class="crm-detail-disclosure-title">Task info</span></summary><div class="crm-detail-disclosure-content"><div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;"><code>${taskIdEsc}</code><button type="button" class="crm-btn-secondary crm-btn-sm" data-copy-id="${taskIdEsc}" aria-label="Copy task ID ${taskIdEsc} to clipboard"><span class="crm-copy-feedback">Copy ID</span></button></div><div class="crm-muted" style="font-size: 12px;">Revision: ${taskRevEsc}</div></div></details>`;
            renderPredecessorChips();
        }
