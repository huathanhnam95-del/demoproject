...
async function boot() {
  ...
  const audio = createAudioController({
    onChange: (state) => { runtime.audioState = { ...state, takeId: runtime.audioState.takeId }; render(); },
    onElapsed: (state) => { runtime.audioState = { ...state, takeId: runtime.audioState.takeId }; updateElapsedNodes(); },
  ...
  function render() {
    ...
    const listeningQuestion = runtime.draft.view === 'question' && currentQuestion()?.type === 'fill' ? currentQuestion() : null;
    if (listeningQuestion && runtime.listeningState.questionId !== listeningQuestion.questionId) {
      runtime.listeningState = { questionId: listeningQuestion.questionId, rate: runtime.listeningState.rate || 1, audioUrl: null };
    }
    ...
    if (previousListeningAudio && nextListeningAudio && previousListeningAudio.dataset.questionId === nextListeningAudio.dataset.questionId && previousListeningAudio.dataset.logicalSrc === nextListeningAudio.dataset.logicalSrc) {
      nextListeningAudio.replaceWith(previousListeningAudio);
    } else if (previousListeningAudio) {
      previousListeningAudio.dataset.mediaResolverRequestId = '';
      previousListeningAudio.pause();
    }
  ...
  async function persistRecording(question, result, takeId, fixture = false) {
    ...
    const adoptPlaybackUrl = () => {
      // The audio controller owns result.url and revokes it on teardown. Keep
      // the saved take's player on a separate URL for its full visible life.
      const playbackUrl = URL.createObjectURL ? URL.createObjectURL(result.blob) : result.url || '';
      const oldUrl = runtime.recordingUrls.get(question.questionId);
      if (oldUrl && oldUrl !== playbackUrl && URL.revokeObjectURL) URL.revokeObjectURL(oldUrl);
      runtime.recordingUrls.set(question.questionId, playbackUrl);
    };
    if (!runtime.storage) {
      adoptPlaybackUrl();
  ...
  function wireListeningAudio(transient = {}) {
    ...
    const isCurrent = () => element.isConnected && runtime.draft.view === 'question' && runtime.draft.activeQuestionId === questionId;
    ...
    if (element.dataset.etListeningBound !== 'true') {
      ...
      element.addEventListener('error', () => { if (isCurrent()) { runtime.listeningState = { ...runtime.listeningState, audioUrl: false }; render(); } }, { once: true });
    }
    updateListeningNodes(element);
    if (previous?.playing) element.play().catch(() => {});
    if (!element.getAttribute('src') && element.dataset.mediaResolverStarted !== 'true') {
      element.dataset.mediaResolverStarted = 'true';
      const resolver = window.MediaUrlResolver;
      if (!resolver || typeof resolver.loadAudio !== 'function') {
        runtime.listeningState = { ...runtime.listeningState, audioUrl: false };
        render();
        return;
      }
      resolver.loadAudio(element, element.dataset.logicalSrc, { mode: 'Entrance-Test' }).then((resolvedUrl) => {
        if (resolvedUrl && isCurrent()) runtime.listeningState = { ...runtime.listeningState, audioUrl: resolvedUrl };
      }).catch(() => {
        if (isCurrent()) { runtime.listeningState = { ...runtime.listeningState, audioUrl: false }; render(); }
      });
    }
  }
