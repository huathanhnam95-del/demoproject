...
function groupButton({ question, item, copy, currentQuestionId = '', action = 'nav-question' }) {
...
function renderTask({ draft, question, questions, sections, summary, copy, audioState, recordingUrl = '', listeningState = {} }) {
  ...
  const passage = `<div class="et-passage"><p>${question.type === 'speaking' ? escapeHtml(question.text) : questionParts(question, draft, copy)}</p></div>`;
...
function renderApp({ draft, sections = [], questions = [], summary, currentQuestion, copy, persistenceState = 'saved', audioState = {}, recordingUrl = '', listeningState = {}, recovery = null, submitting = false }) {
