...
export function requiredPlayers(state) {
  return Object.values(state.players).filter(player => !state.online || player.id === 'p0' || player.connected);
}
...
export function presentationReadiness(state, roomId) {
  ...
  const missing = requiredPlayers(state).filter(player => {
