export class PresentationViewModel {
  ...
  async connect({ replaceExisting = false } = {}) { const ticket = await this.transport.ticket(this.room.roomId); this.connection = await this.transport.connect(ticket.ticket, replaceExisting); return this.connection; }
