...
export class PraatAPI {
     ...
     */
    /**
     * Ask the backend to wake the V3 recognizer.
     *
     * The recognizer scales to zero and pays a 46-52s model load on its first
     * request. Calling this when Pronounce mode opens moves that load into the
     * time a learner spends reading the word, so their analysis request lands
     * on a warm instance. Fire-and-forget: a failed warm-up is never surfaced.
     *
     * @returns {Promise<void>}
     */
    warmV3() {
        return fetch(`${this.backendUrl}/warm/v3`, { method: 'POST', keepalive: true })
            .then(() => undefined)
            .catch(() => undefined);
    }
