...
export function buildComparisonViewModel(comparison) {
    const columns = ['v2', 'v3'].map((version) => {
        ...
        return {
            ...
            status: envelope.status === 'available' ? 'available' : 'unavailable',
            reason: envelope.status === 'available' ? null : normalizeComparisonReason(envelope.reason),
            analysis,
            syllableCount: analysisCount(analysis, syllables),
            confidence: analysisConfidence(analysis),
