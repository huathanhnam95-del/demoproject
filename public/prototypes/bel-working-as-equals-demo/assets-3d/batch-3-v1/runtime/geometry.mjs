...
export function block(THREE, w, h, d, place = {}, bevel = BEVEL) {
  ...
  // instead of reading as rounded pebbles. A zero bevel builds a plain box:
  // pixel-mask cells are already crisp and do not need 60 triangles each.
  const b = Math.min(bevel, 0.1 * Math.min(w, h, d));
  if (b <= 0.0006) return transformed(THREE, new THREE.BoxGeometry(w, h, d), place);
...
// Baked vertical ambient tint for STATIC pieces only: vertices low on the
// object are darkened, so wall/floor junctions, recesses and the inside of the
// channel read without extra lights. Never applied to a movable object, and
// never a stand-in for a real cast shadow.
export function verticalTint(THREE, geometry, y0, y1, tintAtY0, tintAtY1) {
  const position = geometry.getAttribute('position');
  const existing = geometry.getAttribute('color');
  const colors = new Float32Array(position.count * 3);
  for (let i = 0; i < position.count; i++) {
    const t = Math.min(1, Math.max(0, (position.getY(i) - y0) / (y1 - y0 || 1)));
    const value = (tintAtY0 + (tintAtY1 - tintAtY0) * t) * (existing ? existing.getX(i) : 1);
...
export function maskBlocks(THREE, rows, cellSize, thickness) {
  ...
  const push = (x0, x1, z) => parts.push(block(THREE, (x1 - x0 + 1) * cellSize, thickness, cellSize,
    { position: [((x0 + x1) / 2 - (w - 1) / 2) * cellSize, 0, (z - (h - 1) / 2) * cellSize] }, 0));
  for (let z = 0; z < h; z++) {
    let start = -1;
    for (let x = 0; x <= w; x++) {
