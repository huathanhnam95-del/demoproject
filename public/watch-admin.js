    ...
    async function init() {
        try {
            ...
            if (user) {
                ...
                // Client-side admin gate (server-verified; also bootstraps Firestore isAdmin flag)
                try {
                    ...
                    if (!res.ok || !result || !result.success || !result.isAdmin) {
                        showNotification('Access denied. Admin privileges required.', 'error');
                        setTimeout(() => window.location.href = 'index.html', 2000);
                        return;
                    }
                } catch (e) {
                    console.warn('[Admin] Failed to verify admin via server:', e);
                    showNotification('Access check failed. Please try again.', 'error');
                    setTimeout(() => window.location.href = 'index.html', 2000);
                    return;
                }
