...
const WatchMode = (function () {
    ...
    async function init() {
        ...
        isInitialized = true;
        Logger.log('[WatchMode] Initialized');
    }
    ...
    async function loadVideos() {
        try {
            ...
            Logger.log(`[WatchMode] Loaded ${videos.length} videos`);
            renderVideoGrid(videos);
        } catch (error) {
            Logger.error('[WatchMode] Error loading videos:', error);
    ...
    async function selectVideo(videoId) {
        ...
        if (!currentVideo) {
            Logger.error('[WatchMode] Video not found:', videoId);
            return;
        }
    ...
    async function initializePlayer(url) {
        if (window.YouTubePlayer) {
            ...
            Logger.error('[WatchMode] YouTubePlayer module not loaded');
        }
    }
    ...
    function onPlayerReady() {
        Logger.log('[WatchMode] Player ready');
    ...
    function onTimeUpdate(currentTime) {
        ...
        if (skipFirstTimeUpdate) {
            ...
            Logger.log('[WatchMode] First time update, synced lastCheckedTime to:', currentTime);
            return;
        }
    ...
    function pauseAndResetForTabSwitch() {
        ...
        Logger.log('[WatchMode] Paused for tab switch, lastCheckedTime:', lastCheckedTime);
    }
    ...
    async function loadQuestions(videoId) {
        ...
        try {
            if (window.firebase && window.firebase.firestore) {
                ...
                Logger.log(`[WatchMode] Loaded ${questions.length} questions for video ${videoId}`);
            }
        } catch (error) {
            Logger.error('[WatchMode] Error loading questions:', error);
        }
