...
/**
 * Kokoro TTS Batch Audio Generator — Highlight Correct Summary (HCS)
 *
 * Reads HCS.xlsx, selects 3 distinct voices per question (US Male, US Female, UK Male/Female),
...
const EXCEL_PATH = path.join(ROOT_DIR, 'public', 'database', 'HCS', 'HCS', 'HCS.xlsx');
const AUDIO_DIR = path.join(ROOT_DIR, 'public', 'database', 'HCS', 'audio');
const MANIFEST_PATH = path.join(AUDIO_DIR, 'manifest.json');
const ERROR_LOG_PATH = path.join(ROOT_DIR, 'kokoro_hcs_errors.log');
...
async function processQuestion(question, manifest, stats) {
  ...
  for (const voice of voices) {
    const filename = `HCS_${qId}_${voice.id}.mp3`;
...
async function main() {
  console.log('=== Kokoro TTS Batch Generator — HCS ===');
