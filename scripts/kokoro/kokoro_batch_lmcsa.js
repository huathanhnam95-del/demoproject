...
/**
 * Kokoro TTS Batch Audio Generator — Listening Multiple Choice Single Answer (LMCSA)
 *
 * Reads LMCSA.xlsx, selects 3 distinct voices per question (US Male, US Female, UK Male/Female),
 * generates audio files using local Kokoro, and writes manifest.json.
 */
...
const EXCEL_PATH = path.join(ROOT_DIR, 'public', 'database', 'LMCSA', 'LMCSA', 'LMCSA.xlsx');
const AUDIO_DIR = path.join(ROOT_DIR, 'public', 'database', 'LMCSA', 'audio');
const MANIFEST_PATH = path.join(AUDIO_DIR, 'manifest.json');
const ERROR_LOG_PATH = path.join(ROOT_DIR, 'kokoro_lmcsa_errors.log');
...
async function processQuestion(question, manifest, stats) {
  ...
  for (const voice of voices) {
    const filename = `LMCSA_${qId}_${voice.id}.mp3`;
    ...
    try {
      const buffer = await generateAudio(transcript, voice.id, SPEED);
      fs.writeFileSync(filepath, buffer);
      stats.generated++;
      console.log(`  ✓ ${filename} (${voice.name})`);
    } catch (err) {
      stats.failed++;
...
async function main() {
  console.log('=== Kokoro TTS Batch Generator — LMCSA ===');
