...
const { createNativeVoiceProvider, createLiveUsageDescriptor } = require('./native-provider');

// Service-side assembly keeps the deployable Functions tree self-contained.
async function composeVoice(options) {
    return composeDataInputVoice({ ...options, createVoiceProvider: createNativeVoiceProvider, createLiveUsageDescriptor });
}
