...
async function composeProjects({ config, nativeCredentials }) {
    ...
    try {
        ...
        const budget = createProjectsBudgetService({ db, accessService, engineeringMode: false, ...(native ? { nativeMode: true, nativePolicy: native.policy, pricingRegistry: native.pricingRegistry, providerAdapters: { gemini: native.accountingAdapter } } : {}) });
        ...
        return { authenticate: token => auth.verifyIdToken(token, true), sessionService, ledger: budget.ledger,
            ...
            features: native ? { projects: { engineeringOnly: false, provider: 'gemini', model: 'gemini-3.1-flash-live-preview', admission: async scope => {
                ...
                return { model: 'gemini-3.1-flash-live-preview', purpose: 'planning', context: context.context.mode === 'create_project' ? { mode: 'create_project' } : { projectId: context.context.project.id }, boundsVersion: native.policy.versionId, request: { kind: 'live', inputBytes: Buffer.byteLength(JSON.stringify(context)), audioBytes: 3840000, maxOutputTokens: 512 } };
            } } } : {},
            close: () => app.delete()
        };
