...
async function selectTask(page, taskId = TASK) { await view(page, 'kanban'); await page.locator(`[data-task-open="${taskId}"]`).first().click(); await page.locator('#projects-task-schedule').waitFor(); await page.waitForFunction(id => window.projectsViewsController?.getState()?.selectedTaskId === id, taskId); assert.match(await page.locator('#projects-board-detail-body').innerText(), new RegExp(taskId)); }
