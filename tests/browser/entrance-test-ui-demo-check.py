...
class FixtureHandler(SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path: str) -> str:
        ...
        if clean == "/__fixtures__/js/media-url-resolver.js":
            return str(PUBLIC / "js" / "media-url-resolver.js")
...
def candidate_case(page, base: str, output: Path, results: dict, label: str):
    ...
    try:
        page.wait_for_selector("[data-view='question'][data-question-id='vocab_q1']", timeout=10000)
    except PlaywrightTimeoutError as error:
        state = page.evaluate("""() => ({view: document.querySelector('[data-view]')?.dataset.view,
          question: document.querySelector('[data-view=question]')?.dataset.questionId,
          navParts: document.querySelectorAll('[data-action=nav-part]').length,
          notice: document.querySelector('#et-footer-status')?.innerText})""")
        raise AssertionError(f"Vocabulary navigation did not settle: {state}") from error
    ...
    page.wait_for_function("document.querySelector('#et-listening-audio')?.src.includes('/synthetic/listening.wav')")
    assert_true(any("/Entrance-Test.json" in url for url in requests), "Listening audio did not use the media catalog")
    assert_true(not any("/database/Entrance%20Test/Listening%20Q1.mp3" in url for url in requests), "Listening audio requested the missing local file directly")
    page.wait_for_function("document.querySelector('#et-listening-audio')?.duration > 0")
    page.locator("[data-audio-action='toggle']").click()
    page.wait_for_function("document.querySelector('#et-listening-audio')?.currentTime > 0.05")
    page.locator("#et-listening-audio").evaluate("audio => audio.pause()")
    ...
    page.locator("[data-action='next-question']").click()
    page.wait_for_selector("[data-question-id='listen_write_q2']")
    page.wait_for_function("document.querySelector('#et-listening-audio')?.src.includes('/synthetic/listening2.wav')")
    assert_true("Listening Q2.mp3" in page.locator("#et-listening-audio").get_attribute("data-logical-src"), "Q2 reused Q1's logical audio source")
    page.locator("[data-action='previous-question']").click()
    page.wait_for_selector("[data-question-id='listen_write_q1']")
    page.wait_for_function("document.querySelector('#et-listening-audio')?.src.includes('/synthetic/listening.wav')")
...
def audio_case(page, base: str, output: Path, fake_audio: Path, results: dict):
    ...
    page.locator(".et-native-audio").evaluate("audio => audio.play()")
    page.wait_for_function("document.querySelector('.et-native-audio')?.currentTime > 0.05")
    page.locator(".et-native-audio").evaluate("audio => audio.pause()")
    page.locator("[data-mic-action='continue']").click()
    page.wait_for_selector("[data-question-id='speaking_q1']")
    page.locator("[data-record-action='start']").click()
    page.wait_for_selector("[data-record-action='stop']")
    page.wait_for_timeout(700)
    page.locator("[data-record-action='stop']").click()
    try:
        page.locator(".et-recording-take audio").wait_for(timeout=5000)
    except PlaywrightTimeoutError as error:
        state = page.evaluate("""() => ({view: document.querySelector('[data-view]')?.dataset.view,
          question: document.querySelector('[data-question-id]')?.dataset.questionId,
          recorder: document.querySelector('.et-recorder')?.innerText,
          footer: document.querySelector('#et-footer-status')?.innerText})""")
        raise AssertionError(f"Speaking recording was not saved: {state}") from error
    page.locator(".et-recording-take audio").evaluate("audio => audio.play()")
    page.wait_for_function("document.querySelector('.et-recording-take audio')?.currentTime > 0.05")
    page.locator(".et-recording-take audio").evaluate("audio => audio.pause()")
    page.wait_for_timeout(600)
    page.reload()
    page.wait_for_selector("[data-question-id='speaking_q1']")
    page.locator(".et-recording-take audio").wait_for(timeout=10000)
    page.locator(".et-recording-take audio").evaluate("audio => audio.play()")
    page.wait_for_function("document.querySelector('.et-recording-take audio')?.currentTime > 0.05")
...
def host_case(page, base: str, output: Path, results: dict):
    ...
    assert_true(skins == 4, f"Evaluator exposes {skins} skins, expected A, B, C and D")
    ...
    assert_true(page.locator("[data-skin='a'],[data-skin='b'],[data-skin='c']").count() == 3, "Historical design controls are missing")
    assert_true("/160" in page.locator(".et-guide").inner_text(), "Four-design completion does not require 160 scores")
    ...
    try:
        with sync_playwright() as playwright:
            ...
            for label, viewport in viewports.items():
                ...
                media_base = "https://storage.googleapis.com/listening-tasks-3ae34-practice-media/"
                context.route(media_base + "catalogs/pub-20260918-all-media/Entrance-Test.json", lambda route: route.fulfill(
                    status=200, content_type="application/json", headers={"Access-Control-Allow-Origin": "*"},
                    body=json.dumps({"assets": {
                        "public/database/Entrance Test/Listening Q1.mp3": {"key": "synthetic/listening.wav"},
                        "public/database/Entrance Test/Listening Q2.mp3": {"key": "synthetic/listening2.wav"}
                    }})
                ))
                context.route(media_base + "synthetic/listening.wav", lambda route: route.fulfill(
                    status=200, content_type="audio/wav", headers={"Access-Control-Allow-Origin": "*"}, body=fake_audio.read_bytes()
                ))
                context.route(media_base + "synthetic/listening2.wav", lambda route: route.fulfill(
                    status=200, content_type="audio/wav", headers={"Access-Control-Allow-Origin": "*"}, body=fake_audio.read_bytes()
                ))
                def new_page():
