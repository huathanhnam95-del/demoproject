...
async function getWorkbookQuestions() {
  const xlsxPath = path.join(process.cwd(), 'public', 'database', 'HCS', 'HCS', 'HCS.xlsx');
...
function getChoiceCard(page, choiceText) {
  return page.locator('#hcs-choices-container .hcs-choice-card', { hasText: choiceText }).first();
}
...
async function assertScore(page, expectedScore, totalCorrectChoices, expectedHeader) {
  const resultBox = page.locator('#hcs-result-box');
  await page.waitForFunction(() => {
    const box = document.getElementById('hcs-result-box');
...
async function retryAndAssertReset(page) {
  await page.locator('#hcs-retry-btn').click();

  const resetState = await page.evaluate(() => {
    const cards = Array.from(document.querySelectorAll('#hcs-choices-container .hcs-choice-card'));
...
(async () => {
  ...
  try {
    ...
    // Exercise the learner click path: Listening skill filter -> HCS card.
    ...
    await page.waitForFunction(() => {
      ...
      const card = document.getElementById('mode-btn-hcs');
      return button?.classList.contains('is-active') &&
        card &&
        getComputedStyle(card).display !== 'none';
    }, { timeout: 10000 });

    const hcsCard = page.locator('#mode-btn-hcs');
    await hcsCard.click();
    
    // Wait for container to become active and visible
    await page.waitForFunction(() => {
      const panel = document.getElementById('mode-hcs');
    ...
    await page.waitForFunction(() => {
      const prompt = document.getElementById('hcs-question-prompt')?.textContent || '';
    ...
    await page.locator('#hcs-v7-next-btn').click();
    await page.waitForFunction((firstTitle) => {
      const pill = document.getElementById('hcs-v7-question-pill')?.textContent || '';
      return !pill.includes(firstTitle);
    }, firstQuestion.title, { timeout: 5000 });
    await page.locator('#hcs-v7-prev-btn').click();
    await page.waitForFunction((firstTitle) => {
      const pill = document.getElementById('hcs-v7-question-pill')?.textContent || '';
      return pill.includes(firstTitle);
    }, firstQuestion.title, { timeout: 5000 });
    ...
    await page.locator('#hcs-v7-question-pill').click();
    await page.waitForFunction(() => {
      const sheet = document.getElementById('hcs-v7-sheet');
      return sheet?.classList.contains('is-open') && sheet.getAttribute('aria-hidden') === 'false';
    }, { timeout: 5000 });

    await page.locator('#hcs-v7-jump-search').fill('__no_matching_question__');
    await page.waitForFunction(() => {
      const list = document.getElementById('hcs-v7-jump-list')?.textContent || '';
      return list.includes('No matching questions');
    }, { timeout: 5000 });

    await page.locator('#hcs-v7-jump-search').fill(String(target.id));
    await page.waitForFunction((expectedTitle) => {
      const list = document.getElementById('hcs-v7-jump-list')?.textContent || '';
      return list.includes(expectedTitle);
    }, target.title, { timeout: 5000 });
    await page.locator('#hcs-v7-jump-list [data-index]', { hasText: target.title }).first().click();
    await page.waitForFunction(() => {
      const sheet = document.getElementById('hcs-v7-sheet');
      return !sheet?.classList.contains('is-open') && sheet.getAttribute('aria-hidden') === 'true';
    }, { timeout: 5000 });

    pillText = await page.locator('#hcs-v7-question-pill').textContent();
    ...
    const choicesCount = await page.locator('#hcs-choices-container .hcs-choice-card').count();
    ...
    const speedGroup = page.locator('.hcs-speed-group');
    assert.equal(await speedGroup.locator('.hcs-speed-btn.is-active').textContent(), '1.0x', 'Default speed should be 1.0x');
    await speedGroup.locator('.hcs-speed-btn[data-speed="1.25"]').click();
    assert.equal(await speedGroup.locator('.hcs-speed-btn.is-active').textContent(), '1.25x', 'Speed should change to 1.25x');

    const voiceOptions = page.locator('#hcs-voice-select option');
    ...
    const initialAudioSrc = await page.locator('#hcs-audio-element').evaluate((audio) => audio.currentSrc || audio.src);
    assert(initialAudioSrc.includes(`/database/HCS/audio/${target.id}/`), `Audio src should point to target question folder: ${initialAudioSrc}`);
    if (voiceValues.length > 1) {
      await page.locator('#hcs-voice-select').selectOption(voiceValues[1]);
      await page.waitForFunction((previousSrc) => {
        const audio = document.getElementById('hcs-audio-element');
    ...
    const submitBtn = page.locator('#hcs-submit-btn');
    ...
    const transcriptText = await page.locator('#hcs-passage-text').textContent();
    assert.equal(transcriptText.trim(), target.transcript.trim(), 'Audio transcript should match target transcript');

    const explanationContent = await page.locator('#hcs-explanation-content').innerHTML();
