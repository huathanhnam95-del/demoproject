...
async function getWorkbookQuestions() {
  const xlsxPath = path.join(process.cwd(), 'public', 'database', 'LMCSA', 'LMCSA', 'LMCSA.xlsx');
  ...
  assert(questions.length > 0, 'Expected at least one LMCSA question in the workbook with generated explanations');
  return questions;
}

function validateWorkbookAssets(questions) {
  const manifestPath = path.join(process.cwd(), 'public', 'database', 'LMCSA', 'audio', 'manifest.json');
  assert(fs.existsSync(manifestPath), 'Expected LMCSA audio manifest to exist');
  ...
  for (const question of questions) {
    ...
    if (voices) {
      ...
      for (const voice of voices) {
        const audioPath = path.join(process.cwd(), 'public', 'database', 'LMCSA', 'audio', id, voice.file || '');
...
function getChoiceCard(page, choiceText) {
  return page.locator('#lmcsa-choices-container .lmcsa-choice-card', { hasText: choiceText }).first();
}
...
async function assertScore(page, expectedScore, totalCorrectChoices, expectedHeader) {
  const resultBox = page.locator('#lmcsa-result-box');
  await page.waitForFunction(() => {
    const box = document.getElementById('lmcsa-result-box');
...
async function retryAndAssertReset(page) {
  await page.locator('#lmcsa-retry-btn').click();

  const resetState = await page.evaluate(() => {
    const cards = Array.from(document.querySelectorAll('#lmcsa-choices-container .lmcsa-choice-card'));
...
(async () => {
  ...
  try {
    ...
    // Exercise the learner click path: Listening skill filter -> LMCSA card.
    ...
    await page.waitForFunction(() => {
      ...
      const card = document.getElementById('mode-btn-lmcsa');
      return button?.classList.contains('is-active') &&
        card &&
        getComputedStyle(card).display !== 'none';
    }, { timeout: 10000 });

    const lmcsaCard = page.locator('#mode-btn-lmcsa');
    await lmcsaCard.click();
    
    // Wait for container to become active and visible
    await page.waitForFunction(() => {
      const panel = document.getElementById('mode-lmcsa');
    ...
    await page.waitForFunction(() => {
      const prompt = document.getElementById('lmcsa-question-prompt')?.textContent || '';
      const choices = document.querySelectorAll('#lmcsa-choices-container .lmcsa-choice-card');
    ...
    let pillText = await page.locator('#lmcsa-v7-question-pill').textContent();
    ...
    await page.locator('#lmcsa-v7-next-btn').click();
    await page.waitForFunction((firstTitle) => {
      const pill = document.getElementById('lmcsa-v7-question-pill')?.textContent || '';
      return !pill.includes(firstTitle);
    }, firstQuestion.title, { timeout: 5000 });
    await page.locator('#lmcsa-v7-prev-btn').click();
    await page.waitForFunction((firstTitle) => {
      const pill = document.getElementById('lmcsa-v7-question-pill')?.textContent || '';
      return pill.includes(firstTitle);
    }, firstQuestion.title, { timeout: 5000 });
    ...
    await page.locator('#lmcsa-v7-question-pill').click();
    await page.waitForFunction(() => {
      const sheet = document.getElementById('lmcsa-v7-sheet');
      return sheet?.classList.contains('is-open') && sheet.getAttribute('aria-hidden') === 'false';
    }, { timeout: 5000 });

    await page.locator('#lmcsa-v7-jump-search').fill('__no_matching_question__');
    await page.waitForFunction(() => {
      const list = document.getElementById('lmcsa-v7-jump-list')?.textContent || '';
      return list.includes('No matching questions');
    }, { timeout: 5000 });

    await page.locator('#lmcsa-v7-jump-search').fill(String(target.id));
    await page.waitForFunction((expectedTitle) => {
      const list = document.getElementById('lmcsa-v7-jump-list')?.textContent || '';
      return list.includes(expectedTitle);
    }, target.title, { timeout: 5000 });
    await page.locator('#lmcsa-v7-jump-list [data-index]', { hasText: target.title }).first().click();
    await page.waitForFunction(() => {
      const sheet = document.getElementById('lmcsa-v7-sheet');
      return !sheet?.classList.contains('is-open') && sheet.getAttribute('aria-hidden') === 'true';
    }, { timeout: 5000 });

    pillText = await page.locator('#lmcsa-v7-question-pill').textContent();
    ...
    const choicesCount = await page.locator('#lmcsa-choices-container .lmcsa-choice-card').count();
    ...
    const speedGroup = page.locator('.lmcsa-speed-group');
    assert.equal(await speedGroup.locator('.lmcsa-speed-btn.is-active').textContent(), '1.0x', 'Default speed should be 1.0x');
    await speedGroup.locator('.lmcsa-speed-btn[data-speed="1.2"]').click();
    assert.equal(await speedGroup.locator('.lmcsa-speed-btn.is-active').textContent(), '1.2x', 'Speed should change to 1.2x');

    const voiceOptions = page.locator('#lmcsa-voice-select option');
    ...
    const initialAudioSrc = await page.locator('#lmcsa-audio-element').evaluate((audio) => audio.currentSrc || audio.src);
    assert(initialAudioSrc.includes(`/database/LMCSA/audio/${target.id}/`), `Audio src should point to target question folder: ${initialAudioSrc}`);
    if (voiceValues.length > 1) {
      await page.locator('#lmcsa-voice-select').selectOption(voiceValues[1]);
      await page.waitForFunction((previousSrc) => {
        const audio = document.getElementById('lmcsa-audio-element');
    ...
    const submitBtn = page.locator('#lmcsa-submit-btn');
    ...
    const transcriptText = await page.locator('#lmcsa-passage-text').textContent();
    assert.equal(transcriptText.trim(), target.transcript.trim(), 'Audio transcript should match target transcript');

    const explanationContent = await page.locator('#lmcsa-explanation-content').innerHTML();
    ...
    try {
      const successScreenshotPath = path.join(process.cwd(), 'tests', 'browser', 'lmcsa-success-screenshot.png');
    ...
    console.log('LMCSA browser check passed successfully.');
  } catch (err) {
    try {
      const screenshotPath = path.join(process.cwd(), 'tests', 'browser', 'lmcsa-failure-screenshot.png');
