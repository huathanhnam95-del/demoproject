...
async function synthetic(page) {
  await page.evaluate(async () => {
    ...
    fixtureStyle.textContent = '.pte-fixture-background,.guest-toast{display:none!important} #pte-test-panel{max-width:1280px;margin:100px auto 40px}';
...
(async () => {
  ...
  try {
    ...
    for (const width of [1440, 390]) {
      ...
      await page.screenshot({ path: path.join(out, `recording-${width}.png`), fullPage: true });
      await page.locator('#pte-next-speak').click();
      await page.getByRole('button', { name: 'Next question', exact: true }).click();
      ...
      await page.evaluate(() => scrollTo(0, 0));
      await page.screenshot({ path: path.join(out, `complete-${width}.png`), fullPage: true });
      ...
      await page.evaluate(() => scrollTo(0, 0));
      await page.screenshot({ path: path.join(out, `prep-${width}.png`), fullPage: true });
