...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => /Break down|Phân tích|Understand|Hiểu|unavailable|không khả dụng/i.test(document.querySelector('#essay-guided-content')?.textContent || '') || !document.querySelector('#essay-guided-unavailable')?.hidden, null, { timeout: 7000 });
    ...
    assert.equal(await page.locator('#essay-guided-language').inputValue(), 'en');
