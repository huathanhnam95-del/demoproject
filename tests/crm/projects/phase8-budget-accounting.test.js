...
function nativeFixture(enabled = true) {
    ...
    const options = { ...f.options, engineeringMode: false, nativeMode: enabled, nativePolicy: createNativeAccountingPolicy({ versionId: 'native-test-estimate', models: [model], deriveEstimatedQuantities: () => ({ inputText: '0', outputText: '1' }) }), featureAdapters: { projects: adapter, 'crm-data-input': adapter }, pricingRegistry: [{ versionId: 'native-test-price', provider: 'gemini', model, serviceTier: 'standard', effectiveAt: '2026-01-01T00:00:00Z', expiresAt: '2028-01-01T00:00:00Z', ratesNano: { inputText: '0', outputText: '1000000000', inputAudio: '0' } }], providerAdapters: { gemini: { native: true, normalizeEvidence({ evidence }) { if (!f.evidenceSet?.has(evidence)) reject('UNTRUSTED_EVIDENCE', 'Trusted transport required.'); return evidence; } } } };
