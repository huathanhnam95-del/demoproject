/* Synthetic API for PR11 30/500/5000 measurements; synthetic latency, no backend capacity claim. Never contacts a backend. */
(function (root) {
    'use strict';
    root.createProjectsPerformanceFixture = async function (bindings, count = 30) {
        const elements = Object.fromEntries(Object.entries(bindings).map(([key, id]) => [key, document.getElementById(id)]));
        const h = { actor: 'a', role: 'Owner', denied: '', calls: [], holds: [], failures: [], projects: ['p', 'q'], messages: [], membershipRevision: 1, calendarStatus: 'verified' };
        const copy = value => JSON.parse(JSON.stringify(value));
        h.tasks = ['p', 'q'].flatMap(projectId => Array.from({ length: count }, (_, i) => ({
            ...
            startDate: '2026-09-01', dueDate: '2026-09-20', values: { notes: 'Stored' }
        })));
        ...
        h.api = async (url, options) => {
            ...
            const call = { url, method, body, dispatchMs: performance.now() }; h.calls.push(call);
            ...
            let result;
            const project = { id: projectId, name: 'Dự án tiếng Việt', lifecycle: 'active', revision: 1, structureRevision: 1, schemaRevision: 1, membershipRevision: h.membershipRevision };
            if (url.includes('/changes/hydrate')) {
                const rows = h.messages.filter(m => body.messageIds.includes(m.id));
                result = { messages: copy(rows), unavailableMessageIds: body.messageIds.filter(id => !rows.some(m => m.id === id)) };
            } else if (options) {
                ...
                result = { project, membership: { role: h.role }, tasks: copy(url.includes('/views?') ? tasks.slice(0, 200) : tasks), matchingTaskCount: tasks.length, hasMore: url.includes('/views?') && tasks.length > 200,
                    sections: [{ id: 's', title: 'Công việc', rank: '0/1' }], columns: [{ id: 'notes', type: 'text', label: 'Ghi chú' }],
                ...
                const rows = h.messages.filter(m => m.taskId === u.pathname.split('/')[5]);
                const offset = Number(u.searchParams.get('cursor') || 0), end = offset + 50;
                result = { messages: copy(rows.slice(offset, end)), hasMore: end < rows.length, nextCursor: end < rows.length ? String(end) : null };
            }
            else if (url.includes('/links')) result = { links: [] };
            else if (url.includes('/calendar')) result = { calendar: { revision: 1, days: [], coverage: { status: h.calendarStatus } } };
            ...
            call.ackMs = performance.now(); return result;
        };
        const getCurrentUser = () => ({ uid: h.actor });
        h.discussion = root.CrmProjectsDiscussion.createController({ elements, getCurrentUser, apiFetchJson: h.api }); h.discussion.init(); root.CrmProjectsDiscussion.setSelection = h.discussion.setSelection;
        ...
        h.contextSubscriptions = 0;
        const subscribe = h.board.subscribeContext;
        h.board.subscribeContext = listener => { h.contextSubscriptions++; const remove = subscribe(listener); return () => { h.contextSubscriptions--; remove(); }; };
        ...
        h.elements = elements;
        h.remote = async change => {
            const ok = await h.board.applyRemote(change);
            if (ok === false || await h.discussion.applyRemote(change) === false) return false;
            if (h.views.reconcileRemote) return h.views.reconcileRemote(change);
            if ((change.changes.length || change.authorityChanged) && h.views.getState().view !== 'board') await h.views.refresh();
