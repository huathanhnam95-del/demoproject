...
(async () => {
  ...
  assert.equal(publicIndex.prompts.length, 3, 'all workbook rows should be indexed');
  ...
  assert.equal(avi.hasSoundChanges, false, 'Avi Loeb row should not show Level 3 sound changes');
  ...
  assert.equal(coverageJson.promptCount, 3, 'coverage should include all prompts');
  ...
  assert.equal(coverageReport.coverage.promptCount, 3, 'report script should read the generated index');
